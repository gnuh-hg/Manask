...
export async function initI18n() {
    if (Object.keys(LANGS).length === 0) {
        // Thử load cache từ localStorage trước (hoạt động khi offline)
        try {
            const cached = localStorage.getItem('i18n_cache');
            if (cached) LANGS = JSON.parse(cached);
        } catch {}

        // Fetch bản mới — nếu thành công thì cập nhật cache
        try {
            ...
            LANGS = { en, vi };
            localStorage.setItem('i18n_cache', JSON.stringify(LANGS));
        } catch {
            // Offline và chưa có cache → dùng object rỗng, t() trả về key name
            if (Object.keys(LANGS).length === 0) LANGS = { en: {}, vi: {} };
        }
    }

...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.addEventListener('projectSelected', function(e) {
        ...
        const h1p = container.querySelector('h1 p');
        h1p.removeAttribute('data-i18n');
        h1p.innerHTML = nameProject;
    ...
    document.addEventListener('projectUpdated', function(e) {
        ...
        if (projectId === id) {
            ...
            utils.setUserItem('selectedProjectName', name);
        }
    });
    ...
    document.addEventListener('projectDeleted', function(e) {
        ...
        if (projectId === id) {
            ...
            const _overlay = document.getElementById('taskDetailOverlay');
            if (panel) { panel.classList.remove('active'); _overlay?.classList.remove('active'); }

            container.querySelector('h1').style.display = 'none';
            const h1p = container.querySelector('h1 p');
            h1p.setAttribute('data-i18n', 'home.task_manager_title');
            h1p.innerHTML = t('home.task_manager_title');
            showEmptyState('noProject');
        }
    });

    const TASK_SKELETON_HTML = Array.from({ length: 6 }, () => `
        <div class="task-skeleton-item">
            <div class="tsk-line tsk-line--title"></div>
            <div class="tsk-line tsk-line--sub"></div>
            <div class="tsk-footer">
                <div class="tsk-badge"></div>
                <div class="tsk-meta"></div>
            </div>
        </div>`).join('');
    ...
    function showTaskLoadError() {
        ...
        errorEl.innerHTML = `
            ...
            <div class="task-load-error-text">
                <p>${t('home.task_offline_title')}</p>
                <span>${t('home.task_offline_desc')}</span>
            </div>
            <button class="task-retry-btn">${t('home.sidebar_offline_retry')}</button>`;
        errorEl.querySelector('.task-retry-btn').addEventListener('click', () => {
            removeTaskLoadError();
            loadData();
        });
        taskList.insertAdjacentElement('afterend', errorEl);
    }

    function removeTaskLoadError() {
        container.querySelector('.task-load-error')?.remove();
    }
    ...
    async function loadData() {
        ...
        try {
            ...
            taskList.innerHTML = '';
            if (!response.ok) {
                utils.showError(t('home.msg_load_error'));
                return;
            }
    ...
    async function loadSortSettings() {
        try {
            ...
            utils.showWarning(t('home.sort_filter_offline_warn'));  // offline
        }
    }
    ...
    function attachEvents(item, data){
        ...
        item.addEventListener('click', (e) => {
            ...
            // Process slider
            const processSlider = panel.querySelector('#processSlider');
            const processValue  = panel.querySelector('#processValue');
            const processSect   = panel.querySelector('.tdp-progress-section');
            const val = data.progress ?? 0;
            if (processSlider) {
                processSlider.value = val;
                processSlider.style.setProperty('--slider-fill', val + '%');
            }
    ...
    container.querySelector('h1 .h1-actions .btn-done').addEventListener('click', async (e) => {
        if (!projectId && !utils.TEST) {
            utils.showWarning(t('home.msg_select_project'));  // validation
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                taskData.push(_item);
                renderItem(_item);
                utils.showSuccess(t('home.msg_task_created'));
            } else {
                utils.showError(t('home.msg_task_create_error'));
            }
        } catch (err) {
            utils.showError(t('home.msg_task_create_error'));
        }
    ...
    taskDetailOverlay?.addEventListener('click', () => {
        closeDetailPanel(document.querySelector('.task-detail-panel'));
    });
    ...
    document.addEventListener('click', function(e) {
        ...
        // Clicking the task-detail-overlay is handled separately above
        if (taskDetailOverlay && taskDetailOverlay.contains(e.target)) return;
        ...
        priorityBadge.addEventListener('click', function(e) {
            ...
            priorityDebounceTimer = setTimeout(() => {
                utils.fetchWithAuth(
                    ...
                    { enableQueue: true },
                    utils.generateId(), 1
                ).catch(() => utils.showError(t('home.msg_priority_error')))
            }, 500);
        });
    }
    ...
    class TaskDetailDatePicker extends TaskDatePicker {
        ...
        _updateProgress() {
            // Progress is now manually controlled via the slider (activeData.process).
            // No auto-calculation from dates.
        }
        ...
        _syncBackend() {
            ...
            dateDebounceTimer = setTimeout(() => {
                utils.fetchWithAuth(
                    ...
                    { enableQueue: true },
                    utils.generateId(), 1
                ).catch(() => utils.showError(t('home.msg_date_error')));
            }, 500);
        }
    }
        ...
        notesTextarea.addEventListener('input', function () {
            ...
            notesDebounceTimer = setTimeout(() => {
                ...
                utils.fetchWithAuth(
                    ...
                    { enableQueue: true },
                    utils.generateId(), 1
                ).catch(() => utils.showError(t('home.msg_notes_error')));
            }, 500);
        });
    }

    // ========== PROCESS SLIDER ==========
    const processSlider = document.querySelector('#processSlider');
    ...
    if (processSlider) {
        processSlider.addEventListener('input', function () {
            ...
            const processSect  = document.querySelector('.tdp-progress-section');

            // Update UI immediately
            this.style.setProperty('--slider-fill', val + '%');
            if (processValue) processValue.textContent = val + '%';
            if (processSect) processSect.classList.toggle('is-complete', val === 100);

            // Update task item progress bar
            if (activeItem) {
                const fill         = activeItem.querySelector('.progress-bar-fill');
                const percent      = activeItem.querySelector('.progress-percent');
                const taskProgress = activeItem.querySelector('.task-progress');
                if (fill)         fill.style.width    = val + '%';
                if (percent)      percent.textContent = val + '%';
                if (taskProgress) taskProgress.classList.toggle('progress-full', val === 100);
            }

            if (!activeData) return;
            activeData.progress = val;
            ...
            processDebounceTimer = setTimeout(() => {
                utils.fetchWithAuth(
                    ...
                    { enableQueue: true },
                    utils.generateId(), 1
                ).catch(() => utils.showError(t('home.msg_process_error')));
            }, 500);
        });
    }
    ...
    if (btnDeleteTask) {
        btnDeleteTask.addEventListener('click', async function(e) {
            ...
            try {
                ...
                } else utils.showError(t('home.msg_task_delete_error'));
            } catch (err) {
                utils.showError(t('home.msg_task_delete_error'));
            }
        });
    ...
    function sendReorder() {
        ...
        }).catch(() => utils.showError(t('home.msg_reorder_error')));
        }, 500);
    }
    ...
    function reapplySortFilter() {
        ...
        const visible = filterConditions.length === 0
            ...
            : taskData.filter(item => {
                const results = filterConditions.map(f => testFilter(item, f));
                return filterLogic === 'AND' ? results.every(Boolean) : results.some(Boolean);
              });

        if (sortConditions.length > 0) {
            visible.sort((a, b) => {
                for (const s of sortConditions) {
                    const cmp = compareTaskField(a, b, s.field, s.asc);
                    if (cmp !== 0) return cmp;
                }
                return 0;
            });
        ...
        taskData.forEach(item => {
            const el = taskList.querySelector(`.task[data-id="${item.id}"]`);
            if (el) el.style.display = visibleIds.has(String(item.id)) ? '' : 'none';
        });
        ...
        if (visible.length === 0) showEmptyState('noTask');
        else hideEmptyState();
    }

    function testFilter(item, f) {
        const { field, operator, value, valueFrom, valueTo } = f;
        if (field === 'name') {
            const v = (item.name || '').toLowerCase();
            const q = String(value || '').toLowerCase();
            if (operator === 'contains')     return v.includes(q);
            if (operator === 'not_contains') return !v.includes(q);
            if (operator === 'eq')           return v === q;
            if (operator === 'not_eq')       return v !== q;
        }
        if (field === 'priority') {
            const vals = Array.isArray(value) ? value : [];
            if (operator === 'in')     return vals.includes(item.priority);
        ...
        if (field === 'time_spent') {
            ...
            if (operator === 'between') {
                const fromSec = parseTimeToSeconds(valueFrom);
                const toSec   = parseTimeToSeconds(valueTo);
                return (fromSec === null || itemSec >= fromSec) &&
                       (toSec   === null || itemSec <= toSec);
            }
            const filterSec = parseTimeToSeconds(value);
            if (filterSec === null) return true;
            if (operator === 'eq')     return itemSec === filterSec;
            if (operator === 'not_eq') return itemSec !== filterSec;
            if (operator === 'gt')     return itemSec >   filterSec;
            if (operator === 'gte')    return itemSec >=  filterSec;
            if (operator === 'lt')     return itemSec <   filterSec;
            if (operator === 'lte')    return itemSec <=  filterSec;
        }
        return true;
    }
    ...
    // SORT_FIELDS: theo spec — "name"|"start_date"|"due_date"|"time_spent"|"priority"
    ...
    const FIELD_OPERATORS = {
        ...
        priority:    ['in', 'not_in'],
        start_date:  ['eq', 'not_eq', 'gt', 'gte', 'lt', 'lte', 'between'],
        due_date:    ['eq', 'not_eq', 'gt', 'gte', 'lt', 'lte', 'between'],
        time_spent:  ['eq', 'not_eq', 'gt', 'gte', 'lt', 'lte', 'between'],
    };
    ...
    document.getElementById('sf-sort-apply')?.addEventListener('click', async () => {
        ...
        if (!utils.TEST && projectId) {
            try {
                ...
                if (!res.ok) {
                    console.error('[SORT]', res.status, await res.text());
                    utils.showError(t('home.msg_sort_error'));
                } else {
        ...
        function buildValueInput(type, currentVal) {
            if (type === 'date') {
                ...
                inp.addEventListener('blur', () => {
                    ...
                    if (isNaN(num) || num < 0) {
                        utils.showWarning(t('home.filter_error_invalid_hours'));  // validation
                        inp.value = '';
    ...
    document.getElementById('sf-filter-apply')?.addEventListener('click', async () => {
        ...
        if (!utils.TEST && projectId) {
            try {
                ...
                if (!res.ok) {
                    utils.showError(t('home.msg_filter_error'));
                } else {

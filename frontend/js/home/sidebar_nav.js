...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const SIDEBAR_SKELETON_HTML = `
        <div class="sidebar-skeleton">
            <div class="sidebar-skeleton-item">
                <div class="sk-icon"></div>
                <div class="sk-text" style="width:55%"></div>
            </div>
            <div class="sidebar-skeleton-item indent">
                <div class="sk-icon circle"></div>
                <div class="sk-text" style="width:40%"></div>
            </div>
            <div class="sidebar-skeleton-item indent">
                <div class="sk-icon circle"></div>
                <div class="sk-text" style="width:65%"></div>
            </div>
            <div class="sidebar-skeleton-item">
                <div class="sk-icon"></div>
                <div class="sk-text" style="width:45%"></div>
            </div>
            <div class="sidebar-skeleton-item indent">
                <div class="sk-icon circle"></div>
    ...
    function showLoadError() {
        mainListWrapper.innerHTML = `
            <div class="sidebar-load-error">
                ...
                <div class="sidebar-load-error-text">
                    <p>${t('home.sidebar_offline_title')}</p>
                    <span>${t('home.sidebar_offline_desc')}</span>
                </div>
                <button class="sidebar-retry-btn">${t('home.sidebar_offline_retry')}</button>
            </div>`;
        mainListWrapper.querySelector('.sidebar-retry-btn')
    ...
    async function loadData() {
        try {
            ...
            const response = await utils.fetchWithAuth(`${utils.URL_API}/items`, {}, {
                onLoadStart: () => mainListWrapper.insertAdjacentHTML('beforeend', SIDEBAR_SKELETON_HTML),
                onLoadEnd:   () => mainListWrapper.querySelector('.sidebar-skeleton')?.remove(),
            });
            ...
            utils.setUserItem('sidebarLength', mainListWrapper.children.length); // Cache dữ liệu thô để test offline
        } catch (err) {
            mainListWrapper.querySelector('.sidebar-skeleton')?.remove();
            console.error("Lỗi khi load dữ liệu:", err);
            if (err.message !== 'Unauthorized') showLoadError();
            return;
        }
        updateEmptyState();
    }
    ...
    async function saveAllStructure() {
        ...
        saveTimeout = setTimeout(async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    utils.showError('Error saving structure');
    ...
    function attachEvents(item) {
        ...
        if (item.classList.contains('folder-item')) {
            item.querySelector('.item-header').addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (!response.ok) {
                        utils.showError(t('home.msg_folder_status_error'));
        ...
        if (item.classList.contains('project-item-child')) {
            item.addEventListener('click', function(e) {
                ...
                utils.setUserItem('selectedProjectId', projectId);
                utils.setUserItem('selectedProjectName', name);
            
                // Đóng floating card trên mobile khi chọn project (desktop: no-op)
                closeFolderCard();
    ...
    document.querySelector('.modal-box .btn-accept').addEventListener('click', async function() {
        ...
        const newItem = {
            ...
            color: color, 
            parent_id: null, 
            position: utils.getUserItem('sidebarLength') || 0,
            expanded: false
        };
        ...
        try {
            ...
            if (res.ok) {
                ...
                input.value = '';
                closeModals();
                utils.showSuccess(t('home.msg_item_created'));
            } else {
                utils.showError(t('home.msg_create_error'));
            }
        } catch (err) {
    ...
    document.querySelector('.modal-more-box .btn-accept').addEventListener('click', async function() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                closeModals();
                utils.showSuccess(t('home.msg_item_renamed'));
            } else {
                utils.showError(t('home.msg_update_error'));
            }
        } catch (err) {
    ...
    document.querySelector('.btn-delete').addEventListener('click', async function() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                utils.showError(errorData.detail || t('home.msg_delete_error'));
            }
        } catch (err) {
    ...
    // =========================================================
    // --- 6. FOLDER FLOATING CARD ---
    // =========================================================

    const folderCard    = document.getElementById('folderFloatingCard');
    const cardOverlay   = document.getElementById('folderCardOverlay');
    const btnFolders    = document.getElementById('btn-folders');

    function openFolderCard() {
    ...
    btnFolders?.addEventListener('click', (e) => {
        e.stopPropagation();
        folderCard?.classList.contains('active') ? closeFolderCard() : openFolderCard();
    });

    cardOverlay?.addEventListener('click', closeFolderCard);
    document.getElementById('folderCardCloseBtn')?.addEventListener('click', closeFolderCard);

    // Đóng card khi chọn project
    document.addEventListener('projectSelected', closeFolderCard);

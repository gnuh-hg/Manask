...
async function initDonut() {
  ...
  if (utils.TEST) DONUT_DATA = {
    ...
    const section = document.querySelector('.donut-section');
    const ERROR_HTML = `
        <div class="stat-error-wrap">
            ...
            <button class="stat-retry-btn" id="donutRetry">${t('statistics.offline_retry')}</button>
        </div>`;
    try {
      const res = await utils.fetchWithAuth(`${utils.URL_API}/statistic/donut_chart`, {}, {
      ...
      if (err.message !== 'Unauthorized') {
        ...
        document.getElementById('donutRetry')?.addEventListener('click', () => {

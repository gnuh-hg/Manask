...
async function initLineChart() {
  ...
  year: {
    ...
    const section = document.querySelector('.line-section');
    const ERROR_HTML = `
        <div class="stat-error-wrap">
            ...
            <button class="stat-retry-btn" id="lineRetry">${t('statistics.offline_retry')}</button>
        </div>`;
    try {
      const res = await utils.fetchWithAuth(`${utils.URL_API}/statistic/line_chart`, {}, {
      ...
      if (err.message !== 'Unauthorized') {
        ...
        document.getElementById('lineRetry')?.addEventListener('click', () => {
